...
import com.ghostwording.hugsapp.model.GifImages;
...
import com.ghostwording.hugsapp.textimagepreviews.PickHelper;
...
import static com.ghostwording.hugsapp.chatbot.SequenceHandler.ContentType.GIF;
import static com.ghostwording.hugsapp.chatbot.SequenceHandler.ContentType.IMAGE;
import static com.ghostwording.hugsapp.chatbot.SequenceHandler.ContentType.IMAGE_TEXT;
import static com.ghostwording.hugsapp.chatbot.SequenceHandler.ContentType.TEXT;
...
public class SequenceHandler {
    ...
    public interface ContentType {
        ...
        String IMAGE = "Image";
        String GIF = "Gif";
        String GIF_CATEGORY = "GifCategory";
        String IMAGE_CATEGORY = "ImageCategory";
        String TEXT_CATEGORY = "TextCategory";
        String TAB = "Tab";
    }
    ...
    public interface Actions {
        ...
        String CREATE_GIF = "CreateAnimatedEmoji";
        ...
        String EXIT_ONBOARDING = "ExitOnboardingBot";
        String USER_PICK_THEME = "UserPickATheme";
        String USER_PICK_IMAGE = "UserPickImage";
        String USER_PICK_INTENTION = "UserPickIntention";
        String SHOW_FAVORITE_THEME = "ShowFavoriteTheme";
        String SHOW_FAVORITE_IMAGE = "ShowFavoriteImage";
        String SHOW_FAVORITE_INTENTION = "ShowFavoriteIntention";
        String SHOW_MAIN_SCREEN = "ShowMainScreen";
    }
                    ...
                    ApiClient.getInstance().giffyService.getGifByIds(stringBuilder.toString()).enqueue(new Callback<GifResponse>(chatAdapter.getActivity()) {
                        ...
                        public void onDataLoaded(@Nullable GifResponse result) {
                            if (result != null) {
                                ...
                                chatAdapter.addMessage(new ChatMessage(gifImage));
                            }
                            startStep();
                        }
                    });
        ...
        ApiClient.getInstance().botSuggestionsService.getSuggestedImages(step.getParameters().getPath()).enqueue(new Callback<List<PopularImages>>(chatAdapter.getActivity()) {
            ...
            public void onDataLoaded(@Nullable List<PopularImages> result) {
                if (result != null) {
                    ...
                    if (picture != null) {
                        final ChatMessage.BotMessage botMessage = new ChatMessage.BotMessage(picture);
                        chatAdapter.addMessage(new ChatMessage(botMessage));
                    }
                }
                startStep();
        ...
        ApiClient.getInstance().botSuggestionsService.getSuggestedTexts(step.getParameters().getIntentionId()).enqueue(new Callback<List<PopularTexts>>(chatAdapter.getActivity()) {
            ...
            public void onDataLoaded(@Nullable List<PopularTexts> popularTexts) {
                if (popularTexts != null) {
                    ...
                    if (quote != null) {
                        final ChatMessage.BotMessage botMessage = new ChatMessage.BotMessage(quote);
                        chatAdapter.addMessage(new ChatMessage(botMessage));
                    }
                }
                startStep();
    ...
    private void handleImageStep(BotSequence.Step step) {
        String imagePath = step.getParameters().getPath();
        if (step.getParameters().getSource().equals("Internal")) {
            imagePath = PictureService.HOST_URL + imagePath;
        }
        chatAdapter.addMessage(new ChatMessage(new ChatMessage.BotMessage(imagePath)));
        startStep();
    }
        ...
        ApiClient.getInstance().pictureService.getPicturesByPath(imagePath).enqueue(new Callback<List<String>>(chatAdapter.getActivity()) {
            ...
            public void onDataLoaded(@Nullable List<String> result) {
                if (result != null) {
                    ...
                    if (picture != null) {
                        final ChatMessage.BotMessage botMessage = new ChatMessage.BotMessage(PictureService.HOST_URL + picture);
                        chatAdapter.addMessage(new ChatMessage(botMessage));
                        if (showMenu) {
                            chatAdapter.getBotCommandsView().showMessageCommands(new BotCommandsView.MessageCommandsListener() {
                                @Override
